...
describe("networkCollector — fetch", () => {
  ...
  it("drops malformed JSON responses instead of persisting raw sensitive fields", async () => {
    ...
    expect(req!.d.url).toBe(
      "https://api.example.com/bad-json?token=[REDACTED;len=12;charset=mixed]",
    );
    expect(res!.d.body).toBeUndefined();
    expect(res!.d.bodySummary).toMatchObject({
      kind: "json",
...
describe("networkCollector — XHR", () => {
  ...
  it("describes a FormData file part on net.req.file without delaying send()", () => {
    const { events, bus, cleanup } = collect();
    ...
    const fileEvent = events.find(
      (e) => e.k === "net.req.file" && e.d.nameShape !== undefined,
    );
    expect(fileEvent).toBeDefined();
    expect(fileEvent!.d.field).toBe("invoice");
    expect(fileEvent!.d.declaredType).toBe("application/pdf");
