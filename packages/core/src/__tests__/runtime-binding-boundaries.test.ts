...
it("cancels a config wait without aborting shared session registration or starting a late GET", async () => {
  vi.useFakeTimers();
  ...
  const config = fetchRuntimeBindingConfig(
    runtimeBinding,
    `${endpoint}/api/capture-config`,
    controller.signal,
  );
  const failure = expect(config).rejects.toThrow("cancelled");
  ...
  expect(
    fetcher.mock.calls.filter(([url]) => String(url).includes("session/start")),
  ).toHaveLength(1);
});
afterEach(() => {
  vi.useRealTimers();
...
it.each([
  ...
  "rejects a handle outside its endpoint and project scope: %j",
  async (target) => {
    const fetcher = vi.fn();
    const runtimeBinding = createRuntimeBindingHandle({
      endpoint,
...
it.each(["timeout", "abort"])(
  "bounds shared registration on %s without cancelling another caller",
  async (mode) => {
    vi.useFakeTimers();
    ...
    const short = startHeadlessSession({
      ...options,
      sessionId: "short",
      timeoutMs: 20,
      signal: controller.signal,
    });
    const failure = expect(short).rejects.toThrow(
      mode === "timeout" ? "timed out" : "cancelled",
    );
    const other = startHeadlessSession({
      ...options,
      sessionId: "other",
      timeoutMs: 1000,
    });
    ...
    expect(
      fetcher.mock.calls.filter(([url]) =>
        String(url).includes("session/start"),
      ),
    ).toHaveLength(1);
    expect(
    ...
    ).toContain('"sessionId":"other"');
  },
);

it.each([401, 402, 429])(
  "preserves refused session HTTP %s in event delivery",
  async (status) => {
    const fetcher = vi.fn().mockResolvedValue(new Response("{}", { status }));
    vi.stubGlobal("fetch", fetcher);
    const transport = new HttpTransport(endpoint, { authToken: "key" });
    await expect(transport.startSession("session", {})).rejects.toMatchObject({
      status,
    });
    await expect(
      transport.sendEvents([{ t: 1, k: "console", d: {} } as BugEvent]),
