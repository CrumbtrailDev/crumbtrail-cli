...
function boundedColumns(value: unknown): string[] | undefined {
  if (!Array.isArray(value) || value.length === 0 || value.length > MAX_RELATIONAL_ORDER_COLUMNS)
    return undefined;
...
function boundedBooleanVector(value: unknown, length: number): boolean[] | undefined {
...
function canonicalValue(
  ...
  if (depth > MAX_CANONICAL_DEPTH || typeof value !== "object") return undefined;
  ...
  try {
    ...
    if (Array.isArray(value)) {
      ...
      for (let index = 0; index < value.length; index += 1) {
        const encoded = canonicalValue(value[index], depth + 1, seen);
    ...
    for (const key of keys) {
      ...
      const encoded = canonicalValue(descriptor.value, depth + 1, seen);
      if (encoded === undefined) return undefined;
...
function normalizedState(options: {
  ...
  const cached = stateByOptions.get(options);
  if (cached !== undefined) return cached;
  const config = options.relationalOrder;
  let state: RelationalState | null = null;
  try {
    if (!config || !Array.isArray(config.declarations) || config.declarations.length === 0)
    ...
    if (key.length === 0 || key.length > 1024) throw new Error("invalid relational key");
    ...
    for (const declaration of config.declarations) {
      ...
      if (!relationId || !parentTable || !parentColumns || !childTable || !childColumns)
      ...
      if (declaration.constraintTiming === "deferred" && !declaration.deferrable)
    ...
    if (
      new Set(declarations.map((declaration) => declaration.relationIdentity)).size !==
      declarations.length
    )
    ...
    const maxEventsPerRequest =
      requested === undefined || !Number.isFinite(requested)
        ...
        : Math.max(1, Math.min(MAX_RELATIONAL_ORDER_EVENTS_PER_REQUEST, Math.floor(requested)));
    state = { key, declarations, maxEventsPerRequest, countsByRequest: new Map() };
  } catch {
    state = null;
  }
...
function ownParam(params: unknown, key: string): { found: boolean; value?: unknown } {
...
function insertRows(
  ...
  const columns = splitTopLevel(prefix.slice(open + 1, close)).map(unwrapIdentifier);
  if (
    columns.length === 0 ||
  ...
  if (
    rows.length === 0 ||
    (remainder.length > 0 && !/^returning\b/iu.test(remainder))
  )
    return undefined;
  return { target, rows };
}
...
function valueIdentity(
  ...
  if (encoded === undefined) return undefined;
  return digest(state.key, VALUE_DOMAIN, `${declaration.relationIdentity}|${encoded}`);
}

function takeBudget(state: RelationalState, requestId: string): boolean {
  if (state.countsByRequest.size >= 256 && !state.countsByRequest.has(requestId)) {
    const oldest = state.countsByRequest.keys().next().value;
    if (typeof oldest === "string") state.countsByRequest.delete(oldest);
  }
...
function buildEvent(input: {
  ...
  const now = Number.isFinite(input.now) ? Math.round(input.now as number) : Date.now();
  ...
  if (Number.isFinite(startedAt)) event.offsetMs = Math.max(0, now - Number(startedAt));
  return event;
}
  ...
  for (const row of input.rows) {
    for (const declaration of input.state.declarations) {
      ...
      for (const { role, columns } of roles) {
        ...
        if (!identity || !takeBudget(input.state, input.requestId)) return;
