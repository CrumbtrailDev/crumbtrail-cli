...
describe("recurrence identity and the route", () => {
  const bug = (
    detector: string,
    route: string | undefined,
    title = "Database select on sessions was refused (42703)",
  ) =>
    ({
      title,
      representative: {
    ...
    }) as unknown as Pick<DistinctBug, "title" | "representative">;

  it("keeps one identity for a refused statement whether or not a route was captured", () => {
    // Measured live: one refused SELECT produced two issues with byte identical
    // titles and the same detector, because one capture carried a route and the
    // other carried none. An optional field may not fork identity.
    const withRoute = computeDistinctBugSignatures(
      bug("db_statement_failed", "https://shop.example/orders"),
    );
    const withoutRoute = computeDistinctBugSignatures(
      bug("db_statement_failed", undefined),
    );
    expect(withRoute.current).toBe(withoutRoute.current);
  });

  it("keeps one identity for the same statement reached from two pages", () => {
    const fromOrders = computeDistinctBugSignatures(
      bug("db_statement_failed", "/orders"),
    );
    const fromAccount = computeDistinctBugSignatures(
      bug("db_statement_failed", "/account"),
    );
    expect(fromOrders.current).toBe(fromAccount.current);
  });

  it("still separates two HTTP failures on different endpoints", () => {
    // The route IS part of what an HTTP failure is: the same status on two
    // endpoints is two problems, and this must not be collapsed.
    const me = computeDistinctBugSignatures(
      bug("http_error", "/api/me", "HTTP 500"),
    );
    const orders = computeDistinctBugSignatures(
      bug("http_error", "/api/orders", "HTTP 500"),
    );
    expect(me.current).not.toBe(orders.current);
  });

  it("separates two different statements on one page", () => {
    const sessions = computeDistinctBugSignatures(
      bug("db_statement_failed", "/orders", "Database select on sessions was refused (42703)"),
    );
    const orders = computeDistinctBugSignatures(
      bug("db_statement_failed", "/orders", "Database select on orders was refused (42703)"),
    );
    expect(sessions.current).not.toBe(orders.current);
  });
});
