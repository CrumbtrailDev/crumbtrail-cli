...
function req(t: number, requestId: string, url: string): BugEvent {
  return {
    t,
    k: "net.req",
    d: { requestId, m: "GET", url },
  } as unknown as BugEvent;
}

function res(t: number, requestId: string, data: unknown): BugEvent {
  return {
    t,
    k: "net.res",
    d: {
      ...
      st: 200,
      dur: 5,
      body: { ct: "json", bytes: 64, truncated: false, data },
    },
  } as unknown as BugEvent;
}
...
describe("stream_desync", () => {
  it("names a reconnect that skipped a change to the resource", () => {
    const [candidate] = candidatesFor([
      req(500, "a", "/api/orders"),
      res(600, "a", [{ id: 1, status: "pending" }]),
      ...reconnect,
      req(4_000, "b", "/api/orders"),
      res(4_100, "b", [{ id: 1, status: "shipped" }]),
    ]);
  ...
  it("stays quiet about drift when the resource did not move", () => {
    const [candidate] = candidatesFor([
      ...
      ...reconnect,
      req(4_000, "b", "/api/orders"),
      res(4_100, "b", [{ id: 1, status: "pending" }]),
    ]);
  ...
  it("needs a reopen after the drop", () => {
    expect(candidatesFor([sse(1_000, "open"), sse(2_000, "error")])).toHaveLength(
      0,
    );
  });
  ...
  it("compares database rows the stream's own API root read", () => {
    ...
    const [candidate] = candidatesFor([
      req(500, "a", "/api/orders/1"),
      rowRead(550, "a", "pending"),
      res(600, "a", { ok: true }),
      ...reconnect,
      req(4_000, "b", "/api/orders/1"),
  ...
  it("ignores a resource outside the stream's API root", () => {
    const [candidate] = candidatesFor([
      req(500, "a", "/api/inventory"),
      res(600, "a", [{ id: 1, count: 4 }]),
      ...reconnect,
      req(4_000, "b", "/api/inventory"),
      res(4_100, "b", [{ id: 1, count: 9 }]),
    ]);
    expect(candidate.severity).toBe("low");
  });
});
