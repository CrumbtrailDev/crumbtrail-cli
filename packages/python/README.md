...
Configure `CRUMBTRAIL_ENDPOINT` with the HTTPS Crumbtrail origin and `CRUMBTRAIL_INGEST_KEY` with the project's backend ingest key. Create one client **after each server worker forks**. The client rejects missing or malformed configuration. Capture is disabled for every route unless `should_capture` explicitly selects it. Requests must include valid `x-crumbtrail-session-id` and `x-crumbtrail-request-id` headers from an existing browser session.
...
Call `client.close(timeout=5)` from the server's worker shutdown hook. It stops accepting evidence and waits up to five seconds for queued delivery. It returns `False` if work remains. Abrupt termination can lose queued evidence. ASGI lifecycle scopes pass through, so register your shutdown hook explicitly.

The background worker uses a queue of 64 batches, each containing at most 20 events. A request retains at most 198 intermediate events and emits a capture gap when that limit is exceeded. Delivery retries 404, 429, server failures and network failures up to four attempts with five second request timeouts. Redirects are disabled. Other rejection statuses are not retried. `client.sink.dropped` and `client.sink.failed` report overflow or failed delivery counts when using the default sender. Capture failures do not change application response bytes or exceptions. A failure after response transmission begins records `backend.req.error` and retains the emitted HTTP status. An inherited client from another process drops evidence and returns `False` on close, so create the client after worker fork.

JSON request and response bodies retain at most 16 KiB each. The conservative profile retains numbers, booleans, null, short lowercase enums and three letter uppercase units. Sensitive fields and other strings are redacted. Duplicate keys, unsupported object keys, excessive depth and oversized collections are rejected. Bodies have explicit `captured`, `redacted`, `missing`, `invalid` or `truncated` states. Bodies the application does not fully read are marked truncated. Request capture never pre reads a stream. Response streaming remains incremental. Query strings, headers, raw path parameters and exception messages are not exported.
