...
describe("diagnoseCors", () => {
  it("checks the exact non-credentialed browser ingest contract", async () => {
    ...
    expect(fetchImpl.mock.calls[0][1]).toMatchObject({
      ...
      headers: {
        ...
        "Access-Control-Request-Headers": browserIngestRequestHeaders.join(", "),
      },
    });
  ...
  it("names every missing browser ingest header", async () => {
    const result = await diagnoseCors({ endpoint, origin, applicable: true, fetchImpl: (async () => response(204, { "access-control-allow-origin": origin, "access-control-allow-methods": "POST" })) as typeof fetch });
  ...
  it("accepts wildcard origins for the non-credentialed browser ingest request", async () => {
    const result = await diagnoseCors({ endpoint, origin, applicable: true, fetchImpl: (async () => response(204, { "access-control-allow-origin": "*", "access-control-allow-methods": "POST", "access-control-allow-headers": browserIngestRequestHeaders.join(",") })) as typeof fetch });
    expect(result.status).toBe("pass");
  });
