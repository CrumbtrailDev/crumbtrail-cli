...
export function expressEnvPreloadSnippet(keyEnvVar: string): string {
  return [
    ...
    "// file is evaluated). Both candidates are tried, in the order the",
    "// installer picks between them: a bare loadEnvFile() reads .env only, so",
    "// a repo that already had a .env.local got its key written to a file the",
    "// preload never opened, and the middleware posted with no key.",
