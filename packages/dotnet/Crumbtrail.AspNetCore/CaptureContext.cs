...
public class CaptureContext
{
    ...
    public void Stop()
    {
        lock (gate) { SessionId = null; RequestId = null; events.Clear(); DroppedEvents = 0; }
    }
    public void Flush(ICaptureSink sink)
    {
        ...
        lock (gate)
        {
            ...
            batch = events.ToArray(); events.Clear(); DroppedEvents = 0;
        }
    ...
    {
        lock (gate) { if (Active) events.Add(new(Now, kind, payload)); }
    }
